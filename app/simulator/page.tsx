'use client';

import { useState, useEffect, useCallback, useRef } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import Notice from '@/components/Notice';
import { DIET_OPTIONS, isDietValue } from '@/lib/diet-options';
import { buildSimulatorActivities, saveSimulatorSnapshot } from '@/lib/simulator-snapshot';

export default function SimulatorPage() {
  const router = useRouter();
  const [carKmMonthly, setCarKmMonthly] = useState(390);
  const [vehicleSubtype, setVehicleSubtype] = useState('petrol');
  const [electricityKwhMonthly, setElectricityKwhMonthly] = useState(320);
  const [renewablePct, setRenewablePct] = useState(0);
  const [dietPattern, setDietPattern] = useState('mixed');
  const [flightKmYearly, setFlightKmYearly] = useState(1500);
  const [wasteKgMonthly, setWasteKgMonthly] = useState(45);
  const [region, setRegion] = useState('GLOBAL');

  const [simResult, setSimResult] = useState<{
    baselineAnnualKg: number;
    simulatedAnnualKg: number;
    reductionKg: number;
    reductionPct: number;
  } | null>(null);

  const [loading, setLoading] = useState(false);
  const [simError, setSimError] = useState<string | null>(null);
  const [hasBaseline, setHasBaseline] = useState(false);
  const baselineRef = useRef({ carKmMonthly: 390, vehicleSubtype: 'petrol', electricityKwhMonthly: 320, renewablePct: 0, dietPattern: 'mixed', flightKmYearly: 1500, wasteKgMonthly: 45 });
  const requestRef = useRef<AbortController | null>(null);

  const runSimulation = useCallback(async (signal?: AbortSignal) => {
    setLoading(true);
    setSimError(null);
    try {
      const res = await fetch('/api/simulator', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          carKmMonthly,
          vehicleSubtype,
          electricityKwhMonthly,
          renewablePct,
          dietPattern,
          flightKmYearly,
          wasteKgMonthly,
        }),
        signal,
      });

      const data = await res.json().catch(() => null);
      if (res.ok && data) {
        setSimResult(data);
      } else {
        setSimError(
          data?.error === 'EMISSION_FACTOR_NOT_FOUND'
            ? 'We couldn’t price that combination with our current factors. Try a different vehicle or diet option.'
            : 'We couldn’t compute that combination. Check your values and try again.'
        );
      }
    } catch (err) {
      if ((err as Error).name !== 'AbortError') {
        setSimError('Connection problem. Your saved data is safe — try again in a moment.');
      }
    } finally {
      setLoading(false);
    }
  }, [carKmMonthly, vehicleSubtype, electricityKwhMonthly, renewablePct, dietPattern, flightKmYearly, wasteKgMonthly]);

  useEffect(() => {
    fetch('/api/simulator').then((res) => res.ok ? res.json() : null).then((data) => {
      if (!data) return;
      const get = (activityType: string) => data.activities.find((activity: { activityType: string }) => activity.activityType === activityType);
      const car = get('car'); const electricity = get('electricity'); const flight = get('flight'); const waste = get('waste');
      const dietValue = get('diet')?.subtype ?? data.profile?.dietPattern?.toLowerCase() ?? 'mixed';
      const baseline = { carKmMonthly: car?.quantity ?? 0, vehicleSubtype: car?.subtype ?? 'petrol', electricityKwhMonthly: electricity?.quantity ?? 0, renewablePct: 0, dietPattern: isDietValue(dietValue) ? dietValue : 'mixed', flightKmYearly: flight?.quantity ?? 0, wasteKgMonthly: waste?.quantity ?? 0 };
      baselineRef.current = baseline;
      if (data.profile?.region) setRegion(data.profile.region);
      setHasBaseline(true);
      setCarKmMonthly(baseline.carKmMonthly); setVehicleSubtype(baseline.vehicleSubtype); setElectricityKwhMonthly(baseline.electricityKwhMonthly); setRenewablePct(0); setDietPattern(baseline.dietPattern); setFlightKmYearly(baseline.flightKmYearly); setWasteKgMonthly(baseline.wasteKgMonthly);
    }).catch(() => {
      setSimError('We couldn’t load your current activities. The controls below start from typical values.');
    });
  }, []);

  useEffect(() => {
    const timeout = window.setTimeout(() => {
      requestRef.current?.abort();
      const controller = new AbortController(); requestRef.current = controller;
      runSimulation(controller.signal);
    }, 400);
    return () => window.clearTimeout(timeout);
  }, [runSimulation]);

  const baselineTonnes = simResult ? +(simResult.baselineAnnualKg / 1000).toFixed(2) : null;
  const simTonnes = simResult ? +(simResult.simulatedAnnualKg / 1000).toFixed(2) : null;
  const reductionKg = simResult ? simResult.reductionKg : null;
  const reductionPct = simResult ? simResult.reductionPct : null;
  const base = baselineRef.current;

  const handleSaveAsScenario = () => {
    const activities = buildSimulatorActivities({
      carKmMonthly,
      vehicleSubtype,
      electricityKwhMonthly,
      renewablePct,
      dietPattern,
      flightKmYearly,
      wasteKgMonthly,
      region,
    });
    saveSimulatorSnapshot({
      activities,
      createdAt: new Date().toISOString(),
      summary: `${activities.length} activities from your simulator settings`,
    });
    router.push('/scenarios?from=simulator');
  };

  // Prefill target for the reduction plan from the simulated saving (plan slider is 5–60%).
  const planTarget =
    reductionPct !== null && reductionPct > 0 ? Math.min(60, Math.max(5, Math.round(reductionPct))) : 20;

  return (
    <div className="flex flex-col w-full text-on-surface bg-surface-container-lowest">
      {/* Ticker Header */}
      <section className="w-full bg-surface-container-low border-b border-on-surface flex flex-col md:flex-row items-stretch justify-between text-on-surface select-none">
        <div className="px-space-md py-space-xs border-b md:border-b-0 md:border-r border-on-surface flex items-center space-x-space-sm bg-surface-container-lowest">
          <span className="w-2.5 h-2.5 bg-primary animate-pulse" aria-hidden="true"></span>
          <span className="font-label-caps-md text-label-caps-md uppercase tracking-wider text-on-surface font-bold">
            Try changes before you make them
          </span>
        </div>
        <div className="px-space-md py-space-xs border-b md:border-b-0 md:border-r border-on-surface flex items-center flex-1 justify-center bg-surface-container-lowest">
          <span className="font-label-caps-md text-label-caps-md uppercase tracking-wide text-on-surface">
            Uses your real emission factors
          </span>
        </div>
        <div className="px-space-md py-space-xs flex items-center justify-between md:justify-end space-x-space-md bg-secondary-fixed text-on-secondary-fixed">
          <span className="font-label-caps-sm text-label-caps-sm uppercase tracking-widest font-bold" role="status">
            {loading ? 'Calculating…' : 'Ready'}
          </span>
        </div>
      </section>

      {/* Simulator Hero & Body */}
      <div className="p-space-lg md:p-space-xl space-y-space-lg">
        <div className="border-b border-on-surface pb-space-sm flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <span className="font-label-caps-sm uppercase text-primary font-bold tracking-widest">
              What-if explorer
            </span>
            <h1 className="font-headline text-headline-xl uppercase font-bold tracking-tight mt-1">
              What if you changed something?
            </h1>
            <p className="font-body-md text-on-surface-variant max-w-3xl mt-1">
              Move the controls to preview how each change would affect your yearly footprint — calculated with the same factors as your dashboard. Nothing here changes your saved data.
            </p>
          </div>
          <div className="flex items-center space-x-2">
            <Link
              href="/scenarios"
              className="px-space-md py-space-xs bg-surface-container border border-on-surface font-label-caps-md uppercase font-bold hover:bg-on-surface hover:text-white"
            >
              SAVED SCENARIOS MATRIX →
            </Link>
          </div>
        </div>

        {simError && (
          <Notice tone="error">
            {simError}
          </Notice>
        )}

        {/* Result cards */}
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border border-on-surface">
          <div className="p-space-md border-b sm:border-b-0 sm:border-r border-on-surface bg-surface-container-lowest">
            <span className="font-label-caps-sm uppercase text-on-surface-variant font-bold block">Your footprint today</span>
            <div className="font-display text-4xl font-bold mt-1 text-on-surface">
              {baselineTonnes !== null ? <>{baselineTonnes} <span className="text-sm font-headline">t CO₂e/yr</span></> : '…'}
            </div>
            <span className="font-label-caps-sm uppercase text-on-surface-variant font-bold">
              {simResult ? `${Math.round(simResult.baselineAnnualKg).toLocaleString('en-US')} kg total` : 'Loading your current total…'}
            </span>
          </div>

          <div className="p-space-md border-b sm:border-b-0 lg:border-r border-on-surface bg-surface-container-low">
            <span className="font-label-caps-sm uppercase text-primary font-bold block">With these changes</span>
            <div className="font-display text-4xl font-bold mt-1 text-primary">
              {simTonnes !== null ? <>{simTonnes} <span className="text-sm font-headline">t CO₂e/yr</span></> : '…'}
            </div>
            <span className="font-label-caps-sm uppercase text-on-surface-variant font-bold">
              {simResult ? `${Math.round(simResult.simulatedAnnualKg).toLocaleString('en-US')} kg projected` : 'Move a control to preview'}
            </span>
          </div>

          <div className="p-space-md border-b sm:border-b-0 sm:border-r border-on-surface bg-surface-container-lowest">
            <span className="font-label-caps-sm uppercase text-on-surface-variant font-bold block">Difference per year</span>
            <div className={`font-display text-4xl font-bold mt-1 ${reductionKg !== null && reductionKg < 0 ? 'text-error' : 'text-primary'}`}>
              {reductionKg !== null ? <>{reductionKg >= 0 ? `−${Math.round(reductionKg).toLocaleString('en-US')}` : `+${Math.round(-reductionKg).toLocaleString('en-US')}`} <span className="text-sm font-headline">kg/yr</span></> : '…'}
            </div>
            <span className="font-label-caps-sm uppercase text-on-surface-variant font-bold">
              {reductionKg !== null ? `${Math.round(reductionKg / 12).toLocaleString('en-US')} kg / month` : '—'}
            </span>
          </div>

          <div className="p-space-md bg-yellow-accent text-on-surface">
            <span className="font-label-caps-sm uppercase font-bold block">Change</span>
            <div className="font-display text-4xl font-bold mt-1">
              {reductionPct !== null ? (reductionKg !== null && reductionKg >= 0 ? `−${reductionPct.toFixed(1)}%` : `+${Math.abs(reductionPct).toFixed(1)}%`) : '…'}
            </div>
            <span className="font-label-caps-sm uppercase font-bold">
              {reductionKg !== null ? (reductionKg >= 0 ? 'Lower footprint' : 'Higher footprint') : '—'}
            </span>
          </div>
        </div>

        {/* Sliders Grid Split */}
        <div className="grid grid-cols-1 lg:grid-cols-12 border border-on-surface">
          {/* Left Column: Controls (6 cols) */}
          <div className="lg:col-span-6 p-space-lg border-b lg:border-b-0 lg:border-r border-on-surface bg-surface-container-lowest space-y-space-md">
            <div className="flex items-center justify-between border-b border-on-surface pb-space-xs font-label-caps-md uppercase font-bold">
              <span>Your controls</span>
              <button
                type="button"
                onClick={() => {
                  const baseline = baselineRef.current;
                  setCarKmMonthly(baseline.carKmMonthly); setVehicleSubtype(baseline.vehicleSubtype); setElectricityKwhMonthly(baseline.electricityKwhMonthly); setRenewablePct(baseline.renewablePct); setDietPattern(baseline.dietPattern); setFlightKmYearly(baseline.flightKmYearly); setWasteKgMonthly(baseline.wasteKgMonthly);
                }}
                className="min-h-[44px] text-primary hover:underline text-xs"
              >
                Reset to my current setup
              </button>
            </div>
            {!hasBaseline && (
              <p className="font-body-sm text-body-sm text-on-surface-variant">
                Tip: sign in and finish setup so these controls start from your real activities.
              </p>
            )}

            {/* Lever 1: Driving Distance */}
            <div className="border border-on-surface p-space-sm bg-surface-container-low space-y-1">
              <div className="flex justify-between items-center font-label-caps-md uppercase font-bold">
                <label htmlFor="sim-driving">Driving per month</label>
                <span className="text-primary font-headline text-headline-sm">{carKmMonthly} km</span>
              </div>
              <input
                id="sim-driving"
                type="range"
                min="0"
                max="2500"
                step="25"
                value={carKmMonthly}
                onChange={(e) => setCarKmMonthly(Number(e.target.value))}
                className="w-full accent-on-surface cursor-pointer h-2 bg-surface-container-highest"
              />
              <div className="flex justify-between font-label-caps-sm uppercase text-on-surface-variant font-bold">
                <span>0 km (no driving)</span>
                <span>Yours: {base.carKmMonthly} km</span>
                <span>2500 km</span>
              </div>
            </div>

            {/* Lever 2: Vehicle Powertrain */}
            <div className="border border-on-surface p-space-sm bg-surface-container-low space-y-2">
              <div className="flex justify-between items-center font-label-caps-md uppercase font-bold">
                <span id="sim-vehicle-label">Car fuel type</span>
                <span className="text-secondary font-bold font-mono">{vehicleSubtype.toUpperCase()}</span>
              </div>
              <div className="grid grid-cols-4 gap-1 font-label-caps-sm uppercase font-bold" role="group" aria-labelledby="sim-vehicle-label">
                {[
                  { id: 'petrol', label: 'Petrol' },
                  { id: 'diesel', label: 'Diesel' },
                  { id: 'hybrid', label: 'Hybrid' },
                  { id: 'ev', label: 'Electric' },
                ].map((sub) => (
                  <button
                    key={sub.id}
                    type="button"
                    onClick={() => setVehicleSubtype(sub.id)}
                    aria-pressed={vehicleSubtype === sub.id}
                    className={`min-h-[44px] p-1 border border-on-surface text-center ${
                      vehicleSubtype === sub.id ? 'bg-on-surface text-surface-container-lowest' : 'bg-surface-container-lowest hover:bg-surface-container-high'
                    }`}
                  >
                    {sub.label}
                  </button>
                ))}
              </div>
            </div>

            {/* Lever 3: Electricity Usage */}
            <div className="border border-on-surface p-space-sm bg-surface-container-low space-y-1">
              <div className="flex justify-between items-center font-label-caps-md uppercase font-bold">
                <label htmlFor="sim-electricity">Electricity per month</label>
                <span className="text-primary font-headline text-headline-sm">{electricityKwhMonthly} kWh</span>
              </div>
              <input
                id="sim-electricity"
                type="range"
                min="0"
                max="1200"
                step="25"
                value={electricityKwhMonthly}
                onChange={(e) => setElectricityKwhMonthly(Number(e.target.value))}
                className="w-full accent-on-surface cursor-pointer h-2 bg-surface-container-highest"
              />
              <div className="flex justify-between font-label-caps-sm uppercase text-on-surface-variant font-bold">
                <span>0 kWh</span>
                <span>Yours: {base.electricityKwhMonthly} kWh</span>
                <span>1200 kWh</span>
              </div>
            </div>

            {/* Lever 4: Renewable Grid Share */}
            <div className="border border-on-surface p-space-sm bg-surface-container-low space-y-1">
              <div className="flex justify-between items-center font-label-caps-md uppercase font-bold">
                <label htmlFor="sim-renewable">Clean electricity share</label>
                <span className="text-primary font-headline text-headline-sm">{renewablePct}%</span>
              </div>
              <input
                id="sim-renewable"
                type="range"
                min="0"
                max="100"
                step="10"
                value={renewablePct}
                onChange={(e) => setRenewablePct(Number(e.target.value))}
                className="w-full accent-on-surface cursor-pointer h-2 bg-surface-container-highest"
              />
              <div className="flex justify-between font-label-caps-sm uppercase text-on-surface-variant font-bold">
                <span>0% standard grid</span>
                <span>50%</span>
                <span>100% solar / green tariff</span>
              </div>
            </div>

            {/* Lever 5: Diet Profile */}
            <div className="border border-on-surface p-space-sm bg-surface-container-low space-y-2">
              <div className="flex justify-between items-center font-label-caps-md uppercase font-bold">
                <span id="sim-diet-label">Eating habits</span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 font-label-caps-sm uppercase font-bold" role="group" aria-labelledby="sim-diet-label">
                {DIET_OPTIONS.map((d) => (
                  <button
                    key={d.value}
                    type="button"
                    onClick={() => setDietPattern(d.value)}
                    aria-pressed={dietPattern === d.value}
                    className={`min-h-[44px] p-1 border border-on-surface text-center ${
                      dietPattern === d.value ? 'bg-on-surface text-surface-container-lowest' : 'bg-surface-container-lowest hover:bg-surface-container-high'
                    }`}
                  >
                    {d.label}
                  </button>
                ))}
              </div>
            </div>

            {/* Lever 6: Aviation Distance */}
            <div className="border border-on-surface p-space-sm bg-surface-container-low space-y-1">
              <div className="flex justify-between items-center font-label-caps-md uppercase font-bold">
                <label htmlFor="sim-flights">Flights per year</label>
                <span className="text-primary font-headline text-headline-sm">{flightKmYearly} km</span>
              </div>
              <input
                id="sim-flights"
                type="range"
                min="0"
                max="20000"
                step="500"
                value={flightKmYearly}
                onChange={(e) => setFlightKmYearly(Number(e.target.value))}
                className="w-full accent-on-surface cursor-pointer h-2 bg-surface-container-highest"
              />
              <div className="flex justify-between font-label-caps-sm uppercase text-on-surface-variant font-bold">
                <span>0 km (no flights)</span>
                <span>Yours: {base.flightKmYearly} km</span>
                <span>20,000 km</span>
              </div>
            </div>

            {/* Lever 7: Waste */}
            <div className="border border-on-surface p-space-sm bg-surface-container-low space-y-1">
              <div className="flex justify-between items-center font-label-caps-md uppercase font-bold">
                <label htmlFor="sim-waste">Waste per month</label>
                <span className="text-primary font-headline text-headline-sm">{wasteKgMonthly} kg</span>
              </div>
              <input
                id="sim-waste"
                type="range"
                min="0"
                max="200"
                step="5"
                value={wasteKgMonthly}
                onChange={(e) => setWasteKgMonthly(Number(e.target.value))}
                className="w-full accent-on-surface cursor-pointer h-2 bg-surface-container-highest"
              />
              <div className="flex justify-between font-label-caps-sm uppercase text-on-surface-variant font-bold">
                <span>0 kg</span>
                <span>Yours: {base.wasteKgMonthly} kg</span>
                <span>200 kg</span>
              </div>
            </div>
          </div>

          {/* Right Column: Comparison (6 cols) */}
          <div className="lg:col-span-6 p-space-lg md:p-space-xl flex flex-col justify-between bg-surface-container-low">
            <div className="space-y-space-md">
              <div className="border-b border-on-surface pb-space-xs font-label-caps-md uppercase font-bold flex justify-between">
                <span>Today vs these changes</span>
                <span className="text-primary">Your real factors</span>
              </div>

              {/* Comparison rows */}
              <div className="space-y-space-xs font-label-caps-sm uppercase font-bold">
                <div className="p-space-sm bg-surface-container-lowest border border-on-surface flex items-center justify-between">
                  <span className="w-40">Your footprint today</span>
                  <div className="flex-1 mx-space-md hidden sm:flex items-center" aria-hidden="true">
                    <div className="w-full h-0.5 bg-on-surface relative">
                      <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-on-surface"></div>
                    </div>
                  </div>
                  <span className="font-headline text-headline-sm font-bold">{baselineTonnes !== null ? `${baselineTonnes} t` : '…'}</span>
                </div>

                <div className="p-space-sm bg-surface-container-lowest border border-on-surface flex items-center justify-between">
                  <span className="w-40 text-primary">With these changes</span>
                  <div className="flex-1 mx-space-md hidden sm:flex items-center" aria-hidden="true">
                    <div className="w-full h-0.5 bg-primary relative">
                      <div className="absolute right-[25%] top-1/2 -translate-y-1/2 w-3 h-3 bg-primary border border-on-surface"></div>
                    </div>
                  </div>
                  <span className="text-primary font-bold">
                    {simTonnes !== null && reductionPct !== null ? `${simTonnes} t (${reductionPct.toFixed(1)}%)` : '…'}
                  </span>
                </div>
              </div>

              {/* Plain-language summary */}
              <div className="p-space-md bg-surface-container-lowest border border-on-surface space-y-2" aria-live="polite">
                <span className="font-label-caps-md uppercase font-bold text-on-surface block">
                  What this means
                </span>
                {simResult && baselineTonnes !== null && simTonnes !== null && reductionKg !== null && reductionPct !== null ? (
                  <p className="font-body-md text-on-surface leading-relaxed">
                    With these settings, your yearly footprint moves from <strong className="font-bold">{baselineTonnes} t</strong> to <strong className="text-primary font-bold">{simTonnes} t CO₂e</strong>.
                    {reductionKg >= 0 ? (
                      <span> That saves <strong className="text-primary font-bold">{Math.round(reductionKg).toLocaleString('en-US')} kg CO₂e</strong> per year ({reductionPct.toFixed(1)}% less).</span>
                    ) : (
                      <span className="text-error font-bold"> That adds {Math.round(Math.abs(reductionKg)).toLocaleString('en-US')} kg CO₂e per year.</span>
                    )}
                  </p>
                ) : (
                  <p className="font-body-md text-on-surface-variant leading-relaxed">
                    {simError ? 'Fix the issue above and the preview will return.' : 'Calculating your preview…'}
                  </p>
                )}
              </div>
            </div>

            <div className="pt-space-lg border-t border-on-surface flex flex-col sm:flex-row items-center justify-between gap-2">
              <Link
                href={`/reduction-plan?target=${planTarget}`}
                className="min-h-[44px] w-full sm:w-auto px-space-lg py-space-xs bg-primary text-on-primary font-label-caps-md uppercase font-bold border border-on-surface hover:bg-on-surface inline-flex items-center justify-center"
              >
                Use this in my plan →
              </Link>
              <button
                type="button"
                onClick={handleSaveAsScenario}
                className="min-h-[44px] w-full sm:w-auto px-space-md py-space-xs bg-surface-container-lowest text-on-surface font-label-caps-md uppercase font-bold border border-on-surface hover:bg-surface-container-high"
              >
                Save as scenario
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
