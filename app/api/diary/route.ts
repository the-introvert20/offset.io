...
const toEntryData = async (input: z.infer<typeof entrySchema>) => {
  const factor = await emissionFactorService.resolveFactor({ category: input.category, activity: input.activityType, subtype: input.subtype, unit: input.unit });
  return { ...input, date: new Date(input.date), emissionsKg: Number((input.quantity * factor.factor).toFixed(2)) };
};
...
export async function POST(req: Request) {
  try {
    ...
    const entry = await prisma.diaryEntry.create({ data: { userId: user.userId, ...await toEntryData(parsed.data) } });
...
export async function PATCH(req: Request) {
  try {
    ...
    const entry = await prisma.diaryEntry.update({ where: { id: existing.id }, data: await toEntryData(merged) });
